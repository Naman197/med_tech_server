...
const retailOrderData=require('../../data/retailorderdata')
...
exports.showOrdersByPharmacy = async (req, res) => {
    try {
        const retailUserId = req.user.id; // Extracted from the token via middleware
console.log(retailUserId);
        // Filter the orders by the retailUser ID
        const orders = retailOrderData.filter(order => order.retailUser === retailUserId);
